...
const { ApiCompaniesIdGetRoute } = require('../../../../../src/routes');
const {
	AsyncFunction,
	createCompanyObjectStub,
} = require('../../../../_helpers');

describe('ApiCompaniesIdGetRoute', () => {
	...
	beforeEach(() => {
		...
		companiesDao = {
			read: sinon.stub().resolves(company),
		};
		request = { params: { id } };
		reply = { code: sinon.stub() };
		subject = new ApiCompaniesIdGetRoute(companiesDao);
	});
	...
	describe('getOptions()', () => {
		...
		describe('handler()', () => {
			...
			it('should return the existing document from the dao as simple object', async () => {
				...
				expect(companiesDao.read).to.have.been.calledWith(id);
				expect(actual).to.be.an('object');
			...
			it('should transform and throw the not found error from the dao', async () => {
				...
				companiesDao.read.throws(error);
				const { handler } = subject.getOptions();
			...
			it('should catch dao errors and throw a generic one instead', async () => {
				...
				companiesDao.read.rejects(daoError);
				const { handler } = subject.getOptions();
